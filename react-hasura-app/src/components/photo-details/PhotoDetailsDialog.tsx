...
export const PhotoDetailsDialog = ({
  ...
  // for eager loading only when we have the photo id
  ...
  const saveCommentHandler = () => {
    saveComment({
      ...
      update: (cache, result) => {
        // we update the apollo cache manually and mutate result
        // steps -
        // 1. read data from cache
        // 2. modify the data
        // 3. write the data back to cache
        const options = {
      ...
      .then(() => {
        setComment("");
        // refetch && refetch(); // but this is expensive because for a single comment we refetch the whole query
      })
      .catch(console.error);
  };

  // useEffect hook so that if the same photo is opened multiple times
  // then the photo isnt changed hence no new http request would be sent.
  useEffect(() => {
    if (photoId) {
